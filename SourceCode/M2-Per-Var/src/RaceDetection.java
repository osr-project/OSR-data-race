...
public class RaceDetection {
	...
	public HashSet<Integer> racyVars;
	public HashSet<Integer> racyEventsNew;


	public RaceDetection(String filename, boolean dropHB, boolean constructWitness, long startTime) {
...
//		System.out.println("Parsing trace " + this.filename);
	...
	public void init(boolean constructWitness) {
		...
		this.racyVars = new HashSet<>();
		this.racyEventsNew = new HashSet<>();
...
//		}
		...
		int var = this.trace.seqTraces[u.x][u.y].location;
		int e1Id = this.trace.seqTraces[u.x][u.y].inputTraceIndex;
		int e2Id = this.trace.seqTraces[u.x][u.y].inputTraceIndex;

		this.racyVars.add(var);
		this.racyEventsNew.add(Math.max(e1Id, e2Id));
	...
	public int numRacyVars() {
		return this.racyVars.size();
	}

	public int numRacyEvents() {
		return this.racyEventsNew.size();
	}
		...
		for(int i=0; i<frontier.length; i++) {
			if(stops[i] >= frontier[i]) {
				if(top < 0) {
					...
					Pair<Integer, Integer> cur = new Pair<Integer, Integer>(top, frontier[top]);
					Pair<Integer, Integer> next = new Pair<Integer, Integer>(i, frontier[i]);
					if(this.po.existsEdge(next, cur) || (!this.po.existsEdge(cur, next) && this.trace.traceIndex[top][frontier[top]] > this.trace.traceIndex[i][frontier[i]])) {
						top = i;
					}
				}
			}
...
//		System.out.println("Number of potential races: " + potentialRaces.size());
		for(Pair<Pair<Integer, Integer>, Pair<Integer, Integer>> potentialRace : potentialRaces) {
			...
			Event e = this.trace.seqTraces[centerU.x][centerU.y];
			if(!racyVars.contains(e.location)){
	...
	public void getRacesSkip(){
		this.po.buildPORF();
		for (int i = 0; i < this.trace.trace.size(); i++) {
			Pair<Integer, Integer> u = this.trace.trace.get(i);

			Event eventU = this.trace.seqTraces[u.x][u.y];
			if (racyVars.contains(eventU.location)) continue;
			...
			for (int j = 0; j < this.trace.numThreads; j++) {
				...
				if (!u.x.equals(j)) {
					int y = this.getNextConflicting(u, this.po.getPredecessor(new Pair<>(u.x, u.y - 1), j) + 1, j, true);
					while (y > 0 && i > this.trace.traceIndex[j][y]) {
						Pair<Integer, Integer> v = new Pair<>(j, y);

						if (!this.trace.protectedSameLock(u, v)) {
							this.remoteV = v;
							PartialOrderTrace temp = new PartialOrderTrace(this.po);
							if (this.isRace()) {
								this.addRace(this.centerU, this.remoteV);
								foundRace = true;
								long duration = System.currentTimeMillis() - startTime;
								System.out.println(filename + "|" + duration + "|" + this.racyVars.size() + "|M2PerVar");
							}
							this.po = temp;
							if (foundRace) break;
						}
