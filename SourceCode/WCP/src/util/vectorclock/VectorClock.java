...
public class VectorClock implements Comparable<VectorClock> {
	...
	protected int dim;
	protected ArrayList<Integer> clock;

	public VectorClock(int d) {
		this.dim = d;
		this.clock = new ArrayList<Integer>(dim);
	...
	public VectorClock(VectorClock fromVectorClock) {
		...
		this.clock = new ArrayList<Integer>(dim);
		ArrayList<Integer> fromClock = fromVectorClock.getClock();
		for (int ind = 0; ind < fromVectorClock.getDim(); ind++) {
			this.clock.add((Integer) fromClock.get(ind));
		}
	}
	...
	/*
	 * public void inc(int ind) { if(! ((ind < this.dim) && (ind >= 0)) ){ throw new
	 * IllegalArgumentException("You are attempting to access the vector clock with an illegal index"
	 * ); } int new_clock_val = this.clock.get(ind) + 1; this.clock.set(ind,
	 * (Integer) new_clock_val); }
	 */
	...
	public void updateMax2(VectorClock vc) {
		/*
		 * if(! (this.dim == vc.getDim()) ){ throw new
		 * IllegalArgumentException("Mismatch in this.dim and argument.dim"); }
		 */
		for (int ind = 0; ind < this.dim; ind++) {
