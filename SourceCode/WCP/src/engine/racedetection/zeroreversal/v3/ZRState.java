package engine.racedetection.zeroreversal.v3;
...
public class ZRState extends State {
	...
	// 3. Views
	public HashMap<Thread, HashMap<EventType, HashMap<Variable, EfficientLLView<Thread, Pair<VectorClock, VectorClock>>>>> accessInfo;
	...
	public PairComparators.SecondComparator<VectorClock, VectorClock> secondComparatorAccessInfo;
...
//	public HashMap<Thread, HashMap<Lock, Integer>> threadToLockDepth;
	...
	public void initData(HashSet<Thread> tSet) {
		...
		this.accessInfo = new HashMap<Thread, HashMap<EventType, HashMap<Variable, EfficientLLView<Thread, Pair<VectorClock, VectorClock>>>>> ();
		for (Thread t: tSet) {
			this.accessInfo.put(t, new HashMap<EventType, HashMap<Variable, EfficientLLView<Thread, Pair<VectorClock, VectorClock>>>> ());
			this.accessInfo.get(t).put(EventType.READ, new HashMap<Variable, EfficientLLView<Thread, Pair<VectorClock, VectorClock>>> ());
			this.accessInfo.get(t).put(EventType.WRITE, new HashMap<Variable, EfficientLLView<Thread, Pair<VectorClock, VectorClock>>> ());			
		}
		...
		secondComparatorAccessInfo = new PairComparators.SecondComparator<VectorClock, VectorClock> ();
	...
	public int checkAndAddVariable(Variable v){
		if(!variableToIndex.containsKey(v)){
			...
			for(Thread t: threadToIndex.keySet()) {
				...
				this.accessInfo.get(t).get(EventType.READ).put(v, new EfficientLLView<Thread, Pair<VectorClock, VectorClock>> (threadsAccessingVar));
				this.accessInfo.get(t).get(EventType.WRITE).put(v, new EfficientLLView<Thread, Pair<VectorClock, VectorClock>> (threadsAccessingVar));
