...
public class WCPEngine extends RaceDetectionEngine<WCPState, WCPEvent> {
	...
	private HashSet<String> ordredVars;

	private int maxStackSize;

	public WCPEngine(ParserType pType, String trace_folder, boolean forceOrdering, boolean tickClockOnAccess, int verbosity, long startTime) {
		super(pType);
		this.threadSet = new HashSet<>();
		initializeReader(trace_folder);
		this.state = new WCPState(this.threadSet, forceOrdering, tickClockOnAccess, verbosity, trace_folder, startTime);
	...
	public WCPEngine(ParserType pType, String trace_folder, boolean forceOrdering, boolean tickClockOnAccess, int verbosity) {
		super(pType);
		this.threadSet = new HashSet<>();
		initializeReader(trace_folder);
...
//			System.out.println("Time for Phase-1 = " + timeAnalysis + " milliseconds");
		...
		lockEndTimes = accessTimesEngine.lockLast;
		// variableEndTimes = accessTimesEngine.variableLast;
		this.state = new WCPState(this.threadSet, forceOrdering, tickClockOnAccess, verbosity);
		this.state.view.lockThreadLastInteraction = accessTimesEngine.lockThreadLast;
		this.state.existsLockReadVariableThreads = accessTimesEngine.existsLockReadVariableThreads;
		this.state.existsLockWriteVariableThreads = accessTimesEngine.existsLockWriteVariableThreads;
		this.state.variableToReadEquivalenceClass = accessTimesEngine.variableToReadEquivalenceClass;
...
//		System.out.println("Number of races found = " + raceCount);
