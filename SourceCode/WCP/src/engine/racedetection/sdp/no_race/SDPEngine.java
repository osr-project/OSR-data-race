...
public class SDPEngine extends RaceDetectionEngine<SDPState, SDPEvent> {

	private HashMap<String, Long> lockEndTimes;
	private int maxStackSize;

	public SDPEngine(ParserType pType, String trace_folder, boolean forceOrdering,
			boolean tickClockOnAccess, int verbosity) {
		super(pType);
		this.threadSet = new HashSet<Thread>();
		initializeReader(trace_folder);
		this.state = new SDPState(this.threadSet, forceOrdering, tickClockOnAccess,
				verbosity);
		handlerEvent = new SDPEvent();
		...
		this.state.variableToThreadSet = accessTimesEngine.variableToThreadSet;
	}
...
//		System.out.println("Maximum stack size = " + Integer.toString(maxStackSize));
	...
	@Override
	protected boolean skipEvent(SDPEvent handlerEvent) {
		/*
	...
	@Override
	protected void postHandleEvent(SDPEvent handlerEvent) {
