...
public class DCEvent extends RaceDetectionEvent<DCState> {
	...
	public boolean HandleSubRelease(DCState state) {
		...
		for (Variable r : this.readVarSet) {
			VectorClock L_r_l_x = state.getVectorClock(state.lastReleaseLockReadVariable,
					this.getLock(), r);
			L_r_l_x.updateWithMax(L_r_l_x, C_t);
		}

		for (Variable w : this.writeVarSet) {
			VectorClock L_w_l_x = state.getVectorClock(state.lastReleaseLockWriteVariable,
					this.getLock(), w);
	...
	public boolean HandleSubRead(DCState state) {
		...
		for (Lock l : this.getLockSet()) {
			VectorClock writeClock = state.getVectorClock(
					state.lastReleaseLockWriteVariable, l, this.getVariable());
