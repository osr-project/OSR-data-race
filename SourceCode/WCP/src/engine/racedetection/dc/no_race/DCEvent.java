...
public class DCEvent extends RaceDetectionEvent<DCState> {
	...
	public boolean HandleSubRelease(DCState state) {
		...
		VectorClock C_t = state.getVectorClock(state.clockThread, getThread());

		// long startTimeJoins = System.currentTimeMillis();

		for (Variable r : this.readVarSet) {
			if (state.existsLockReadVariableThreads
					.containsKey(this.getLock().getName())) {
				if (state.existsLockReadVariableThreads.get(this.getLock().getName())
						.containsKey(r.getName())) {
					VectorClock L_r_l_x = state.getVectorClock(
							state.lastReleaseLockReadVariable, this.getLock(), r);
					L_r_l_x.updateWithMax(L_r_l_x, C_t);
				}
			}
		}
		...
		for (Variable w : this.writeVarSet) {
			if (state.existsLockWriteVariableThreads
					.containsKey(this.getLock().getName())) {
				if (state.existsLockWriteVariableThreads.get(this.getLock().getName())
						.containsKey(w.getName())) {
					VectorClock L_w_l_x = state.getVectorClock(
							state.lastReleaseLockWriteVariable, this.getLock(), w);
					L_w_l_x.updateWithMax(L_w_l_x, C_t);
				}
			}
		}
		// state.timerReleaseJoins += System.currentTimeMillis() - startTimeJoins;
		...
		// state.timerReleaseTotal += System.currentTimeMillis() - startTimeTotal;
				...
				if (state.existsLockWriteVariableThreads.get(l.getName())
					...
					if (state.existsLockWriteVariableThreads.get(l.getName())
						...
						VectorClock writeClock = state.getVectorClock(
								state.lastReleaseLockWriteVariable, l,
								this.getVariable());
						C_t.updateWithMax(C_t, writeClock);
					}
				}
			}
		}
				...
				if (state.existsLockReadVariableThreads.get(l.getName())
					...
					if (state.existsLockReadVariableThreads.get(l.getName())
						...
						VectorClock readClock = state.getVectorClock(
								state.lastReleaseLockReadVariable, l, this.getVariable());
						C_t.updateWithMax(C_t, readClock);
					}
