...
public class DCView {
	...
	public static Function<TreeClock, Function<ClockEpochPair, Boolean>> compareEpochVC = vc -> clep -> clep
	...
	public TreeClock getMaxLowerBound(Thread tReader, Lock l, Thread tWriter,
			TreeClock ct) {
		checkAndAddLock(l);
		...
		if (mlb.first) {
			...
			return new TreeClock(ct.dim);
		}
	}

	public void updateTopRelease(Lock l, Thread t, TreeClock ht) {
		checkAndAddLock(l);
		ClockEpochPair clockPair = this.view.get(l).get(t).top();
		clockPair.setRelease(new TreeClock(ht));
	}
