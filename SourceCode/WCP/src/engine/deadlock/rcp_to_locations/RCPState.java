...
import engine.deadlock.rcp_to.RCPView;
import event.Lock;
import event.Thread;
...
public class RCPState extends State{
	...
	public HashMap<Lock, ArrayList<Pair<String, Pair<HashSet<Lock>, Epoch>>>> lockAcquisitionHistory; //Lock -> List<(id, LocksHeld, epoch)>
	...
	public void initData(HashSet<Thread> tSet) {
		...
		this.lockAcquisitionHistory = new HashMap<Lock, ArrayList<Pair<String, Pair<HashSet<Lock>, Epoch>>>> ();
	}
	...
	private void checkAndAddLock(Lock l){
		if(!lockSet.contains(l)){
			...
			this.lockAcquisitionHistory.put(l, new ArrayList<Pair<String, Pair<HashSet<Lock>, Epoch>>> ());
		}
	}
