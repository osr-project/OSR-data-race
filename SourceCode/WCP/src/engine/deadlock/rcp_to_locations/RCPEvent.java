package engine.deadlock.rcp_to_locations;
...
//		System.out.println("lockAcquisitionHistory = " + state.lockAcquisitionHistory);
		...
		VectorClock C_t = state.generateTimestamp(this.getThread());
		String this_id = this.getThread() + "." + this.getLocId();
		...
		for(Lock l : locksHeld){
			ArrayList<Pair<String, Pair<HashSet<Lock>, Epoch>>> history_l = state.lockAcquisitionHistory.get(l);
			for(Pair<String, Pair<HashSet<Lock>, Epoch>> triple : history_l){
				String acquire_id = triple.first;
				Pair<HashSet<Lock>, Epoch> pair = triple.second;
				...
				if(locksHeld_l.contains(this.getLock())){
					...
					if(check1 && check2){
						...
						System.out.println(acquire_id + " " + this_id);
					}
				}
			}
		}

		ArrayList<Pair<String, Pair<HashSet<Lock>, Epoch>>> history = state.lockAcquisitionHistory.get(this.getLock());
		Epoch timestamp = state.getEpoch(C_t, this.getThread());
		Pair<HashSet<Lock>, Epoch> lset_epoch = new Pair<HashSet<Lock>, Epoch>(locksHeld, timestamp);
		Pair<String, Pair<HashSet<Lock>, Epoch>> id_lset_epoch = new Pair<String, Pair<HashSet<Lock>, Epoch>>(this_id, lset_epoch);
		history.add(id_lset_epoch);
