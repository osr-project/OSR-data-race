...
public class THBEvent extends AtomicityEvent<THBState> {
	...
	public boolean HandleSubRead(THBState state) {
		...
		boolean getOtherClocks =  true;
		if(!(state.lastAccessThread.get(v) == null)) {
			if(!state.lastAccessThread.get(v).equals(t)) {
				getOtherClocks = false;
			}
		}
		if(getOtherClocks) {
			...
			state.lastAccessThread.put(v, t);
		}
