...
public class THBState extends State {
	...
	public HashMap<Variable, HashSet<Thread>> updateSetVariable_write;
	public HashMap<Variable, HashSet<Thread>> updateSetVariable_read;
	...
	public void initData(HashSet<Thread> tSet) {
		...
		this.updateSetVariable_read = new HashMap<Variable, HashSet<Thread>> ();
		
		this.updateSetVariable_write = new HashMap<Variable, HashSet<Thread>> ();
	}
...
//		checkAndAddVariable(v);
	...
	public int checkAndAddVariable(Variable v){
		if(!variableToIndex.containsKey(v)){
			...
			this.updateSetVariable_read.put(v, new HashSet<Thread> ());
			this.updateSetVariable_write.put(v, new HashSet<Thread> ());
		}
	...
	public boolean handshakeAtEndEvent_Optimized(Thread t) {
		...
		for(Variable v: this.updateSetThread_write.get(t)) {
			...
			this.updateSetVariable_write.get(v).remove(t);
		}
		...
		for(Variable v: this.updateSetThread_read.get(t)) {
			...
			chR_v.updateWithMax(chR_v, C_t);
			this.updateSetVariable_read.get(v).remove(t);
		}
	...
	public void updateSetAtWrite(Thread t, Variable v) {
		int tIndex = this.threadToIndex.get(t);
		VectorClockOpt C_t = this.clockThread.get(tIndex);
		HashSet<Thread> update_set_v = this.updateSetVariable_write.get(v);
		for(Thread u: this.threadToIndex.keySet()) {
			if(!u.equals(t) && threadToNestingDepth.get(u) > 0 && !update_set_v.contains(u)) {
	...
	public void updateSetAtRead(Thread t, Variable v) {
		int tIndex = this.threadToIndex.get(t);
		VectorClockOpt C_t = this.clockThread.get(tIndex);
		HashSet<Thread> update_set_v = this.updateSetVariable_read.get(v);
		for(Thread u: this.threadToIndex.keySet()) {
			if(!u.equals(t) && threadToNestingDepth.get(u) > 0 && !update_set_v.contains(u)) {
				int uIndex = this.threadToIndex.get(u);
				VectorClockOpt C_u_begin = this.clockThreadBegin.get(uIndex);
